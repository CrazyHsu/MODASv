...
drawPhyloPlotR = '''
    ...
    options(warn=-1)
    drawPhyloPlot <- function(nwk_file, out_prefix, drop_lines, group_file, group_sep){
        ...
        if(group_file!="") {
            ...
            groupOTU(circle_tree, groupInfo, 'subpop') + aes(color=subpop) + 
                scale_fill_brewer(palette="Set2") + 
                theme(legend.position = "right")
            ggsave(paste0(out_prefix, '.phylo.tree.pdf'), device='pdf', height=5, width=5)
        }else{
            ggsave(circle_tree, paste0(out_prefix, '.phylo.tree.pdf'), device='pdf', height=5, width=5, useDingbats=FALSE)
        }
    }
'''
...
drawQQplotR = '''
    ...
    drawQQplot <- function(gwas_res, out_prefix, sep, title, dpi, threshold) {
        ...
        CMplot(data, plot.type='q', col='grey30', conf.int.col='gray', signal.col='red', multracks=FALSE, LOG10=TRUE,
               threshold=threshold, signal.cex=0.5, file='jpg', dpi=dpi, file.prefix=out_prefix, main=title)
        sink()
    }
'''
    ...
    drawForestPlot <- function(mr_res, out_prefix, order_by_name, order_by_effect, order_by_group, group, 
        ...
        for(mTrait in unique(data[,"mTrait"])){
            ...
            for(snp in unique(sub_data[,"snp"])){
                ...
                #           xlab('') + ylab('') +
                #           ggtitle(paste(mTrait, snp, sep='_'))
                ggsave(paste(out_prefix, mTrait, snp, "MR", 'forestplot.pdf', sep='_'), plot=p, device='pdf', 
...
drawScatterMrPlotR = '''
    ...
    suppressMessages(library(data.table))
    options(warn=-1)
    drawScatterMrPlot <- function(mr_res, group, out_prefix, sep, height, width){
        data <- fread(mr_res, sep=sep, data.table=getOption("datatable.fread.datatable", FALSE))
        for(mTrait in unique(data[,"mTrait"])){
            sub_data <- data[data$mTrait==mTrait,]
            for(snp in unique(sub_data[,"snp"])){
                ...
                ylimMin <- -log10(0.05/nrow(res)) - 0.5
                ...
                if(group!="None"){
                    scatter_plot <- scatter_plot + geom_label_repel(data=res[which(res$pvalue<=0.05/nrow(res)),], 
                        aes(label=id), color = "white", 
                        point.padding = NA, box.padding = 0.2) + 
                        guides(fill = guide_legend(override.aes = aes(color = NA)))
                }
                ggsave(paste(out_prefix, mTrait, snp, "MR", 'scatterplot.pdf', sep='_'), plot=scatter_plot, 
                    device = 'pdf', height = height,width = width, useDingbats=FALSE)
            }
        }
    }
'''
...
drawScatterPsR = '''
    suppressMessages(library(ggplot2))
    drawScatterPs <- function(ps_file, group_file, out_prefix, ps_type, input_sep, group_sep) {
        ...
        if(ps_type=="pca"){
            x_label <- "PC1" 
            y_label <- "PC2"
            out_file <- paste(out_prefix, 'scatter_PCA.pdf', sep=".")
        }else{
            x_label <- "t-SNE1" 
            y_label <- "t-SNE2"
            out_file <- paste(out_prefix, 'scatter_tSNE.pdf', sep=".")
        }
...
drawHeatmapR = '''
    ...
    drawHeatmap <- function(input, out_prefix, select_rows, select_cols, cluster_rows, cluster_cols, 
        ...
        out_file <- paste(out_prefix, "heatmap.pdf", sep="_")
...
drawBoxplotR = '''
    ...
    drawBoxplot <- function(input_mat_file, snp_file, group_file, selected_genes, selected_snps, selected_strains, to_scale, plot_type, out_prefix){
...
drawNetworkPlotR = '''
    ...
    suppressMessages(library(ggplot2))
    drawNetworkPlot <- function(m_ew, m_hub, fn){
            ...
            suppressMessages(ggsave(fn, device='pdf', useDingbats=FALSE))
    }
'''
...
drawEnrichPlotR = '''
    ...
    drawEnrichPlot <- function(input_list, gene2go_file, bg_sep, plot_type, sigp, adjustp, out_prefix){
